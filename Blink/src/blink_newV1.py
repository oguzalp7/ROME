...
Date: 05/15/2021
Latest draft for Rome-Blink.

The program aims to detect potential anomalies in human brain using eye blinks.
In a random time, a sound occurred to trigger human to blink.
The program keeps the delay between sound time and the first blink.
Also keeps the duration of the blinks.
Plots a graph according to current the data.
And lastly, keeps the data in a MySQL database and also as a JSON file in Patients directory.
...
cap = cv2.VideoCapture(0)  # specify and open camera
fileStream = True
time.sleep(1.0)  # wait for camera to warm up

print("[INFO] sound thread begins...")
# random wait before the startle sound
for i in list(range(random.randint(3, 11)))[::-1]:
    print("[ATTENTION] Program starts in ", i + 1, "seconds.")
    time.sleep(i)

t1 = Thread(target=timer_to_start)  # after random seconds wait startle sound played in parallel thread.
if once:  # since we expect to hear the sound once
    sound_time_ms = time.time()  # put current into a variable
    t1.start()  # start the sound thread
    # print(sound_time)
    once = False  # change once flag to false in order to not to hear the sound again.
while True:
...
patient_name = input("Name: ")  # TODO: fill up the form from gui
patient_surname = input("Surname: ")  # TODO: fill up the form from gui
